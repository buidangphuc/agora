...
The frontend-quality capability defines the `team-frontend` unit/component test tier — the layer the
test pyramid was missing (strong Go backend unit tests and strong platform-e2e coverage, but nothing
in between for the frontend). It gives fast, isolated coverage of the logic the frontend owns: the
Server Actions (`src/features/*/actions.ts`) and the gateway data layer (`src/lib/gateway/*.ts`) that
shapes every request/response between the browser and the gateway. End-to-end user journeys remain in
platform-e2e; this tier is isolated unit tests with the gateway transport and Next.js request APIs
mocked.
