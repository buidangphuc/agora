...
// DBTX is the subset of pgxpool.Pool / pgx.Tx used by the transactional writer,
// so the same status-update SQL runs on either a pool or an open transaction.
type DBTX interface {
...
// a caller-owned transaction (so the event commits atomically with the payment
// write); the claim/mark methods run on the pool and drive the relayer. Mirrors
// team-domain's OutboxStore (ADR-0002 / ADR-0009).
...
// commits or rolls back together with the payment write on that same tx.
func (s *OutboxStore) EnqueueTx(ctx context.Context, tx DBTX, row OutboxRow) error {
	const q = `INSERT INTO payment_outbox_events
...
// same row. Rows are ordered (available_at, created_at) to keep same-order
...
func (s *OutboxStore) MarkPublished(ctx context.Context, eventID string, publishedAt time.Time) error {
	const q = `UPDATE payment_outbox_events
...
func (s *OutboxStore) MarkFailed(ctx context.Context, eventID, reason string, availableAt *time.Time) error {
	...
	var args []any
	if availableAt != nil {
		q = `UPDATE payment_outbox_events
...
// PgTxWriter settles a payment and enqueues its outbox event in one pgx.Tx.
...
// SettleTx opens a transaction, updates the payment status, enqueues the built
// event on the SAME tx, and commits — so the payment row and the outbox row are
// one atomic unit. Any error rolls both back (the deferred Rollback is a no-op
// after a successful Commit).
func (w *PgTxWriter) SettleTx(ctx context.Context, id string, status PaymentStatus, providerRef string, ev SettleFn) (PaymentTransaction, error) {
	tx, err := w.pool.Begin(ctx)
	if err != nil {
		return PaymentTransaction{}, fmt.Errorf("begin tx: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()

	updated, err := updatePaymentStatusTx(ctx, tx, id, status, providerRef)
	if err != nil {
	...
	if err := tx.Commit(ctx); err != nil {
		return PaymentTransaction{}, fmt.Errorf("commit tx: %w", err)
	}
	return updated, nil
}
...
var _ PaymentTxWriter = (*PgTxWriter)(nil)
