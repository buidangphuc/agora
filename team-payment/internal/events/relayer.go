...
// and produces each stored EventEnvelope, marking rows published on success and
// retrying (with backoff) on failure. Delivery is AT-LEAST-ONCE — a row is
// marked published only after a successful produce, so a crash between produce
// and mark re-delivers the row (consumers dedupe on event_id). Mirrors
// team-domain's Relayer.
type Relayer struct {
	store    OutboxClaimer
