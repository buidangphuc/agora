...
// mdRequestID is the human-facing correlation id bridged from the repo's HTTP
// convention onto gRPC metadata. Distinct from W3C `traceparent`, which otel
...
// OTelStatsHandler returns the otelgrpc server stats handler. Prefer this over
// the deprecated otelgrpc unary/stream interceptors — a single stats.Handler
// covers unary AND streaming and reads/writes W3C `traceparent` for you.
//
// ADR-0004: tracing is W3C trace-context via OpenTelemetry; the exporter is
// swappable (OTLP/gRPC by default — see cmd/server/main.go — or stdout/none for
// local dev) without touching handler code.
...
// with OTelStatsHandler() on the server (the stats handler owns `traceparent`;
// this interceptor owns the human-facing id).
