...
// Flipt provider into team-order. Call sites depend only on the tiny Evaluator
...
// FlagCheckoutEnabled is the boolean emergency kill-switch for the checkout /
// purchase saga. Its default is ON (true): an emergency kill-switch is an
// explicit human OFF action during an incident, so a Flipt outage must NOT take
// checkout down (fail-open). See design.md "Kill-switch semantics".
const FlagCheckoutEnabled = "checkout-enabled"

// clientName is the OpenFeature client/logical name for this service.
const clientName = "team-order"
...
// Config is the feature-flag slice of team-order settings.
type Config struct {
